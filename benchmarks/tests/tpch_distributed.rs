...
async fn run_single_query(query_no: usize) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let connect_url = format!("df://{}:{}", cluster.scheduler_host, cluster.scheduler_port);
    ...
    if !data_path.exists() {
        ...
        println!("SKIP: Data path not found at {:?}. Skipping execution.", data_path);
        return Ok(());
    }
    ...
    if let Some(path) = query_file {
        ...
        for sql in queries {
            ...
            println!(
                ...
                query_no, row_count, start.elapsed().as_millis()
            );
        }
    } else {
